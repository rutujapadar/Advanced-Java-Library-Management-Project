...
public class LibraryManagement1 extends JFrame implements ActionListener {
 ...
 reservedBooksButton.addActionListener(this);
    ...
    if (bookId.isEmpty() || bookTitle.isEmpty() || author.isEmpty() || publisher.isEmpty() || yearOfPublicationStr.isEmpty() || isbn.isEmpty() || numOfCopiesStr.isEmpty()) {
        JOptionPane.showMessageDialog(this, "All fields are required");
        return;
    }

    // Check for numeric input for year of publication and number of copies
 ...
 int currentYear = Calendar.getInstance().get(Calendar.YEAR);
        if (yearOfPublication < 1900 || yearOfPublication > currentYear) {
            JOptionPane.showMessageDialog(this, "Year of publication should be between 1900 and the current year");
            return;
        }
    ...
    // Edit book details
    try {
        // Prompt the user to enter the book ID of the book they want to edit
        ...
        String columnName = JOptionPane.showInputDialog(this, "Enter column name to update (book_title, auther, publisher, year_of_publication, isbn, num_of_copies):");
        ...
        String updateQuery = "UPDATE books SET " + columnName.toLowerCase().replace(" ", "_") + " = ? WHERE book_id = ?";
    ...
    try {
        ...
        // If the book is reserved, inform the user and don't delete
        if (reservedResultSet.next()) {
            JOptionPane.showMessageDialog(this, "This book is reserved and cannot be deleted.");
            return;
        }

        // If the book is not reserved, proceed with deletion
        ...
        JOptionPane.showMessageDialog(this, "Error deleting book: " + ex.getMessage());
           ...
           // Add a button for reserving books
if (e.getSource() == reserveButton) {
    ...
    try {
        // Prompt the user to enter the book ID of the book they want to reserve
        ...
        // Retrieve the number of copies of the selected book from the database
        ...
        if (resultSet.next()) {
            ...
            if (numCopies > 0) {
                // Insert a new entry into the reserved books table
                ...
                reserveStatement.setDate(3, new java.sql.Date(System.currentTimeMillis())); // Use the current date as the reservation date
                reserveStatement.executeUpdate();

                // Decrement the number of copies in the books table by 1
